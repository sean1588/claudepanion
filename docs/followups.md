...
- ~~**Build relies on `tsc --watch` being active in dev** (or a fresh `npm run build` in prod) for the watcher to pick up newly-scaffolded companion code.~~ **Resolved** by the v2 watcher fix (dist-mtime gate, debounced retries, `dev:tsc` health probe, structured failure logs) shipped in Phase 2 of `feat/scaffold-consistency`.
